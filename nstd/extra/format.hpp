...
namespace __ {
    ...
    // nstd::benchmark
    template<typename F>
    [[nodiscard]] inline std::string __compose(nstd::benchmark<F>& c) noexcept(true) {
        return std::to_string(c.elapsed()) + "s ";
    }
...
namespace nstd {
    ...
    template<typename T>
    [[nodiscard]] inline std::string format(std::stack<T> data) noexcept(true) {
        return __::__compose(data);
    }

    // converts a vector to a string representation
    template<typename T>
    [[nodiscard]] inline std::string format(const std::vector<T>& data) noexcept(true) {
        return __::__compose(data);
    }

    // converts an unordered_map to a string representation
    template<typename K, typename V>
    [[nodiscard]] inline std::string format(const std::unordered_map<K, V>& data) noexcept(true) {
        return __::__compose(data);
    }

    // converts a map to a string representation
    template<typename K, typename V>
    [[nodiscard]] inline std::string format(const std::map<K, V>& data) noexcept(true) {
        return __::__compose(data);
    }

    // converts a nstd::benchmark to a string representation
    template<typename F>
    [[nodiscard]] inline std::string format(const nstd::benchmark<F>& data) noexcept(true) {
        return __::__compose(data);
    }
}
#endif